...
; 02_FindText() used to find images restored by Base64 text on screen.
...
02_FindText(x, y, w, h, err1, err0, text, ScreenShot=1)
{
  02_xywh2xywh(x,y,w,h,x,y,w,h)
  if (w<1 or h<1)
    return, 0
  bch:=A_BatchLines
  SetBatchLines, -1
  ;--------------------------------------
  02_GetBitsFromScreen(x,y,w,h,Scan0,Stride,ScreenShot,zx,zy)
  ;--------------------------------------
  ...
  Loop, Parse, text, |
  {
    ...
    color:=r1, v:=r2
    StringSplit, r, v, .
    w1:=r1, v:=02_base64tobit(r2), h1:=StrLen(v)//w1
    ...
    ;--------------------------------------
    if (ok:=02_PicFind(mode,color,n,Scan0,Stride
...
02_PicFind(mode, color, n, Scan0, Stride, sx, sy, sw, sh
  ...
  , err1, err0, w1, h1, ByRef allpos)
{
  ...
  if !MyFunc
  {
    ...
    02_MCode(MyFunc, A_PtrSize=8 ? x64:x32)
  }
...
02_xywh2xywh(x1,y1,w1,h1,ByRef x,ByRef y,ByRef w,ByRef h)
{
  SysGet, zx, 76
  SysGet, zy, 77
  SysGet, zw, 78
...
02_GetBitsFromScreen(x, y, w, h, ByRef Scan0, ByRef Stride
...
02_ASCII(s)
{
  ...
  {
    s:=RegExReplace(02_base64tobit(r2),".{" r1 "}","$0`n")
...
; and Use 02_Pic(Text,1) to add the text library to 02_Pic()'s Lib,
; Use 02_Pic("comment1|comment2|...") to get text images from Lib

02_Pic(comments, add_to_Lib=0)
{
  static Lib:=[]
  if (add_to_Lib)
  {
...
02_PicN(Number)
{
  return, 02_Pic( RegExReplace(Number, ".", "|$0") )
}

; Use 02_PicX(Text) to automatically cut into multiple characters

02_PicX(Text)
{
  if !RegExMatch(Text,"\|([^$]+)\$(\d+)\.([\w+/]+)",r)
    return, Text
  w:=r2, v:=02_base64tobit(r3), Text:=""
...
{
  n:=150000
  02_xywh2xywh(-n,-n,2*n+1,2*n+1,x,y,w,h)
  02_GetBitsFromScreen(x,y,w,h,Scan0,Stride,1)
}

02_FindTextOCR(nX, nY, nW, nH, err1, err0, Text, Interval=20)
{
  OCR:="", RightX:=nX+nW-1, 02_ScreenShot()
  While (ok:=02_FindText(nX, nY, nW, nH, err1, err0, Text, 0))
  {
    For k,v in ok
    {
      ...
      else if (x=LeftX)
      {
        ...
        {
          err:=A_Index/100
          if 02_FindText(x, y, w, h, err, err, Text, 0)
          {
            LeftX:=x, LeftY:=y, LeftW:=w, LeftH:=h, LeftOCR:=comment
            Break
          }
          if 02_FindText(LeftX, LeftY, LeftW, LeftH, err, err, Text, 0)
            Break
        }
      }
    }
...
; Use 02_Pic(Text,1) and 02_PicN(number) when using.
; Use 02_PicX(Text) to automatically cut into multiple characters.
; Color position mode is not supported.

02_FindText2(x, y, w, h, err1, err0, text, ScreenShot=1, Interval=20)
{
  02_xywh2xywh(x,y,w,h,x,y,w,h)
  if (w<1 or h<1)
    return, 0
  ...
  ;--------------------------------------
  if (ok:=02_PicFind2(mode,allcolor,n,Scan0,Stride,sx,sy,sw,sh
...
02_PicFind2(mode, color, n, Scan0, Stride, sx, sy, sw, sh
  , ByRef ss, ByRef text, ByRef s1, ByRef s0
...
int __attribute__((__stdcall__)) 02_PicFind(
...
int __attribute__((__stdcall__)) 02_PicFind2(
  int mode, unsigned int c, int n, unsigned char * Bmp
  , int Stride, int sx, int sy, int sw, int sh
  , char * ss, char * text, int * s1, int * s0
