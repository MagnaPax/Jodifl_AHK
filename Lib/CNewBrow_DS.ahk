...
;MsgBox, goToURl_AfterLogIn_IfNeeded 메소드 들어왔음	
...
;MsgBox, 뭔가 엉킨것 같기에 chrome_Profile.txt 파일 내용을 초기화 했습니다.
			...
			driver := return_NewWindowDriver_After_FGLogin(driver, URL)
...
;MsgBox, 내가 가고 싶은 페이지. 로그인 한 뒤 열렸음
	...
	; 만약 현재 페이지가 jodifl admin 페이지라면
	else if(RegExMatch(driver.Url, "imU)jodifl.com")){
		
		; 로그인 페이지라면
		if(RegExMatch(driver.Url, "imU)index.php/admin/index/index")){
			
			
			MsgBox, jodifl.com 로그인 페이지. 만들어야 됨
			
			
		} ; if 닫기 - 로그인 페이지라면	
	} ; if 닫기 - 만약 현재 페이지가 jodifl admin 페이지라면
...
;MsgBox, 내가 가고 싶은 페이지. 로그인 하기 전에 열렸음
	...
	; driver 리턴하기
;MsgBox, 262144, Title, goToURl_AfterLogIn_IfNeeded(URL) 메소드 끝. driver 를 리턴하고 메소드 종료합니다.
	return driver
	...
	; ############################################################################################
	return_NewWindowDriver_After_FGLogin(driver, URL){		
...
;MsgBox, % "프로파일 경로 읽었음" . "`n`n" . "URL : " . url
...
;					MsgBox, It's neither UPSG nor LAS consolidation
			...
			; UPSG 주문일 때
			else if(ShippingMethodStatus == "UPS Ground"){
...
;MsgBox, openNewTab_clickMostTopPO# 함수에 들어왔음
...
;	Sleep 2000
	...
	Loop{
		...
		; 현재 화면에 Customer PO 가 있으면 루프 끝내고 나가기
		if(outerHTMLofTheCustomerPO){
			break
		}
		
		; 못 찾았으면 Input Period 상태를 This Year 로 바꾸기
		if(!outerHTMLofTheCustomerPO){
			
			; Input Period 상태를 This Year 로 바꾸기
			Xpath = /html/body/fg-root/div[1]/fg-secure-layout/div/div[2]/fg-orders/fg-order-search/div/div[2]/div/div/div[1]/select
			driver := changeDropboxStatus(driver, Xpath, 14, "This Year")
			
			; 만약 LOOP 가 짝수번째 도는 상태면 Input Period 상태를 Last 365 Days 로 바꾸기. 
			; 계속 한 값으로 고정되는 것이 아니라 This Year 와 Last 36t Days 를 왔다갔다 하면서 바꾸기 위해 
			If ! Mod( A_Index,2 )
			{
				; Input Period 상태를 Last 365 Days 로 바꾸기
				Xpath = /html/body/fg-root/div[1]/fg-secure-layout/div/div[2]/fg-orders/fg-order-search/div/div[2]/div/div/div[1]/select
				driver := changeDropboxStatus(driver, Xpath, 33, "Last 365 Days")
			}			
...
;MsgBox, Input Period 상태를 Last 365 Days 로 바꿀 예정
...
Magento_LoginOnly(driver){
		...
		; 아이디 입력		
		Xpath = //input[@name='login[username]']
		driver.FindElementByXPath(Xpath).sendKeys(driver.Keys.CONTROL, "a").SendKeys(Loginname) ; 기존에 혹시 자동완성으로 정보가 채워져 있다면 지우기 위해 Ctrl+A 한 뒤 로그인 입력
;MsgBox, 아이디 입력함


		; 비밀번호 입력
		Xpath = //input[@name='login[password]']
		driver.FindElementByXPath(Xpath).sendKeys(driver.Keys.CONTROL, "a").SendKeys(Password).sendKeys(driver.Keys.ENTER) ; Ctrl+A 한 뒤 비밀번호 입력 후 엔터쳐서 로그인하기
;MsgBox, 비밀번호 입력함
