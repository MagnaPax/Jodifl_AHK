...
;~ Text:="|<PDF>*126$32.U000zk0003w0000T00003k0000Q0000300000k0000400k0000K00001U000180000I0000100000k0000A0000200001c0000F7000DsM00312007U7U06E000180000K00006000000000000000000000000007CC001+G000GWU004ci001m+000EYU004C800000000000000000800006"
Text:="|<Desc.>*150$26.w0008U0028tssXGGH8rr4290N0WGnFD3bXa"



if ok:=FindText(2034,268,150000,150000,0,0,Text)
...
;~ if ok:=FindText(2034,268,W+100,H+100,0,0,Text)
if ok:=FindText(2034,268,170,110,0,0,Text)
;~ if ok:=FindText(0,0,2000,800,0,0,Text)
{  
  CoordMode, Mouse
  X:=ok.1, Y:=ok.2, W:=ok.3, H:=ok.4, Comment:=ok.5
  MouseMove, X+W//2, Y+H//2
  MsgBox, % "X : " . X . "`nY : " . Y . "`n`nW : " . W . "`n" . "H : " . H . "`n`n2"
    
}

;~ if ok:=FindText(2034,268,X+200,Y+200,0,0,Text)
;~ if ok:=FindText(2034,268,X,Y,0,0,Text)
;~ if ok:=FindText(2034,268,X+100,Y+100,0,0,Text)
;~ {  
  ;~ CoordMode, Mouse
  ;~ X:=ok.1, Y:=ok.2, W:=ok.3, H:=ok.4, Comment:=ok.5
  ;~ MouseMove, X+W//2, Y+H//2
  ;~ MsgBox, % "X : " . X . "`nY : " . Y . "`n`nW : " . W . "`n" . "H : " . H . "`n`n3"
...
if ok:=FindText(2034,268,150000,150000,0,0,Text)
{
  ...
  ;~ if ok:=FindText(2034,268,150000,150000,0,0,Text)
  ;~ if ok:=FindText(X,Y,100,100,0,0,Text)
  if ok:=FindText(2034,268,W+100,H+100,0,0,Text)
  {
    CoordMode, Mouse
    X:=ok.1, Y:=ok.2, W:=ok.3, H:=ok.4, Comment:=ok.5
    ...
    ;~ if ok:=FindText(2034,268,W,H,0,0,Text){
    if ok:=FindText(2034,268,X+100,Y+100,0,0,Text)
    {
      CoordMode, Mouse
      X:=ok.1, Y:=ok.2, W:=ok.3, H:=ok.4, Comment:=ok.5
