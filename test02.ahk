...
if (ok:=FindText(232-150000//2, 398-150000//2, 150000, 150000, 0, 0, Text))
{
	CoordMode, Mouse
	X_Desc:=ok.1.1, Y_Desc:=ok.1.2, W_Desc:=ok.1.3, H_Desc:=ok.1.4, Comment_Desc:=ok.1.5, X_Desc+=W_Desc//2, Y_Desc+=H_Desc//2
	; Click, %X%, %Y%
	MouseMove, X_Desc, Y_Desc
    Sleep 9000
  }
...
if (ok:=FindText(232-150000//2, 398-150000//2, 150000, 150000, 0, 0, Text))
{
	...
	; Click, %X%, %Y%
	MouseMove, X_Desc, Y_Desc
    MsgBox, % "X_Desc : " . X_Desc . "`nY_Desc : " . Y_Desc . "`n`nW_Desc : " . W_Desc . "`n" . "H_Desc : " . H_Desc . "`n`n1"
    
    ; 두 번째부터 찾기
    Loop{
      
      if (ok:=FindText(X-20, Y, 150000, 150000, 0, 0, Text))
      {
        CoordMode, Mouse
        X_Desc:=ok.1.1, Y_Desc:=ok.1.2, W_Desc:=ok.1.3, H_Desc:=ok.1.4, Comment_Desc:=ok.1.5, X_Desc+=W_Desc//2, Y_Desc+=H_Desc//2

        MouseMove, X_Desc, Y_Desc
        MsgBox, % "X_Desc : " . X_Desc . "`nY_Desc : " . Y_Desc . "`n`nW_Desc : " . W_Desc . "`n" . "H_Desc : " . H_Desc . "`n`n" . A_Index+1
      }
      
      ; 못 찾았으면 화살표 클릭하기
      else{
        
        Text_DownArrow:="|<>*199$16.UUW368CsUT20s810U02"
        if (ok:=FindText(1734-150000//2, 983-150000//2, 150000, 150000, 0, 0, Text_DownArrow))
        {   
          CoordMode, Mouse
...
} ; end끝 - if (ok:=FindText(232-150000//2, 398-150000//2, 150000, 150000, 0, 0, Text)
...
Loop, 30{
  ...
  loop, 8{
    ...
    if (ok:=FindText(1734-150000//2, 983-150000//2, 150000, 150000, 0, 0, DownArrow_Text))
    {
      CoordMode, Mouse
      ;~ X:=ok.1.1, Y:=ok.1.2, W:=ok.1.3, H:=ok.1.4, Comment:=ok.1.5, X+=W//2, Y+=H//2
      X_DownArrow:=ok.1.1, Y_DownArrow:=ok.1.2, W_DownArrow:=ok.1.3, H_DownArrow:=ok.1.4, Comment_DownArrow:=ok.1.5, X_DownArrow+=W//2, Y_DownArrow+=H//2
...
MsgBox, 262144, NO ITEM ERROR, NO ITEM ON THIS SCREEN FOUND
