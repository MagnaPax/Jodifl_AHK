...
B1867 Drop All 에서 중복되는 SO 번호 있어서 테스트 하면 됨
*/
...
Click_btn:
	...
	; Drop All 이 아닌데 아이템번호, 색깔을 입력 안 했으면 프로그램 다시 시작하기
	if(!DropAll){
		if(OOSStyle# == "")
			Reload
		else if(OOSColor == "")
...
; ##########################################################################################################################################################################################################################################
					...
					isIt2ndScreen = 1
					isIt2ndScreen := isThis2ndReadScreen(XY_Arr[2][2nd_index], itemsInfoThatJustReadFromScreen_Arr)
					if(!isIt2ndScreen){
						MsgBox, 262144, title, 마지막 화면을 두 번 읽었음. 루프를 중단하고 나감
						break 3
					}
		...
		if(DropAll || !doesTheItemFound){
			...
			if(XY_Arr.1.MaxIndex() <= 7){
				MsgBox, 262144, title, 마지막 화면에 도달했음. 루프 빠져나감
				break
			}
